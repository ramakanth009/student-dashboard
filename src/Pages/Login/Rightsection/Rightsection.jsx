...
const RightSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      // Check fullstack credentials
      const isFullstackUser = FULLSTACK_CREDENTIALS.some(
        cred => cred.username === formData.email && cred.password === formData.password
      );

      // Check data science credentials
      const isDataScienceUser = DATASCIENCE_CREDENTIALS.some(
        cred => cred.username === formData.email && cred.password === formData.password
      );
